...
void test_fcollect_simple() {
  ...
  *source = me;
  ...
  printf("PE %d: Before fcollect, source = %d\n", me, *source);

  // Perform fcollect
  shmem_team_t world_team = SHMEM_TEAM_WORLD;
  int ret = shmem_int_fcollect(world_team, dest, source, NELEMS);

  shmem_barrier_all();

  // Verify results
  if (ret == 0) {
    printf("PE %d: After fcollect: ", me);
    for (int i = 0; i < npes; i++) {
      printf("%d ", dest[i]);
      if (dest[i] != i) {
        test_passed = 0;
    ...
    printf("PE %d: Fcollect failed with ret = %d\n", me, ret);
    test_passed = 0;
  }
  ...
  if (me != 0) {
    ...
    shmem_int_put(&all_passed[me], &test_passed, 1, 0);
  }
  ...
  if (me == 0) {
    ...
    if (!test_passed) {
      ...
      if (all_tests_passed) {
        printf("Test PASSED: All PEs successfully fcollected data in correct order\n");
      } else {
