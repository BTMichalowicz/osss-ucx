...
#define ALLTOALLS_HELPER_BARRIER_DEFINITION(_algo, _peer, _cond)                \
  inline static int alltoalls_helper_##_algo##_barrier(                         \
    ...
    memcpy(dest_ptr + me_as * dst * nelems,                                    \
           source_ptr + me_as * sst * nelems,                                  \
           nelems);                                                            \
                                                                               \
    ...
    for (int i = 0; i < PE_size; ++i) {                                        \
      ...
      shmem_putmem(dest_ptr + me_as * dst * nelems,                           \
                   source_ptr + peer_as * sst * nelems,                        \
                   nelems, peer);                                              \
    }                                                                          \
                                                                               \
    /* Synchronize */                                                          \
...
#define SHCOLL_ALLTOALLS_DEFINITION(_algo, _type, _typename)                   \
  int shcoll_##_typename##_alltoalls_##_algo(                                  \
    ...
    int ret = alltoalls_helper_##_algo(dest, source,                          \
                                      dst * sizeof(_type),                     \
                                      sst * sizeof(_type),                     \
                                      nelems * sizeof(_type),                  \
