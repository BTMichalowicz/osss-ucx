...
unsigned char enc_chunks[MAX_THREAD_COUNT][MAX_MSG_SIZE + OFFSET];
unsigned char dec_chunks[MAX_THREAD_COUNT][MAX_MSG_SIZE + OFFSET];
unsigned char plain_chunks[MAX_THREAD_COUNT][MAX_MSG_SIZE + OFFSET];
// unsigned char dec_chunks[MAX_THREAD_COUNT][MAX_MSG_SIZE+OFFSET];
...
static void dec_notif_fn(size_t evhdlr_registration_id, pmix_status_t status,
  ...
  shmemu_assert(source_rank != PMIX_RANK_UNDEF,
                "dec_notif_fn: source rank is undefined!");

  DEBUG_SHMEM("decryption pe %d source rank %d address %p og_bytes %lu; "
              "is_nonblocking %d,ciphertext_size %d\n",
              pe, source_rank, (void *)dest, og_size, is_nonblocking,
              cipherlen);
  ...
  int thread_no =
      shmemx_decrypt_single_buffer_omp((unsigned char *)dest, pe, (void *)dest,
                                       source_rank, og_size, (int)cipher_len);
...
int shmemx_encrypt_single_buffer_omp(unsigned char *cipherbuf,
  ...
  *cipherlen = 0; /* Just in case */
  ...
  // #pragma omp parallel for schedule (dynamic) default(none) private(max_data,
  // position, res, local_cipherlen, enc_data) shared(src, dest, openmp_enc_ctx,
  // stdout,stderr, segment_count, data, sbuf, cipherbuf, temp_cipherlen, proc,
  // bytes, gcm_key, enc_chunks, dec_chunks, plain_chunks)
  // num_threads(thread_no)
  for (count = 0; count < segment_count; count++) {
    memset(enc_chunks[count], 0, MAX_MSG_SIZE + OFFSET);
    memset(dec_chunks[count], 0, MAX_MSG_SIZE + OFFSET);
    memset(plain_chunks[count], 0, MAX_MSG_SIZE + OFFSET);
  }
  ...
  // num_threads(thread_no)
  for (count = 0; count < segment_count; count++) {
    memcpy(plain_chunks[count], sbuf + (enc_data * count), enc_data);
  }
  ...
  for (pos = 0; pos < segment_count; pos++) {
    for (count = 0; count < enc_data; count++)
      fprintf(stderr, "%c", plain_chunks[pos][count]);
    fprintf(stderr, "\n");
  }
#pragma omp parallel for schedule(dynamic) default(none)                       \
    ...
    shared(src, dest, openmp_enc_ctx, stdout, stderr, segment_count, data,     \
               ...
               enc_chunks, dec_chunks, plain_chunks) num_threads(thread_no)
  for (count = 0; count < segment_count; count++) {
    ...
    int pos2 = count * data;
    unsigned char *temp_cipher_text =
        &(enc_chunks[count]
                    [0]); // malloc(sizeof(data+AES_TAG_LEN+AES_RAND_BYTES));;
    unsigned char *temp_plain_text = &(plain_chunks[count][0]);

    //  RAND_bytes(tmp_buf, AES_RAND_BYTES);
    EVP_CIPHER_CTX *local_ctx = openmp_enc_ctx[tn];
    RAND_bytes(temp_cipher_text + src, AES_RAND_BYTES);
    enc_data = data;
    ...
    DEBUG_SHMEM(
        ...
        tn, local_ctx, enc_data, max_data, temp_cipher_text,
        (void *)cipherbuf + src, count, enc_data, AES_TAG_LEN, AES_RAND_BYTES);

    if ((res = EVP_EncryptInit_ex(local_ctx, NULL, NULL, NULL,
                                  temp_cipher_text)) != 1) {
    ...
    if ((res = EVP_EncryptUpdate(
             local_ctx, temp_cipher_text + src + AES_RAND_BYTES,
             &local_cipherlen,
             ((const unsigned char *)(temp_plain_text + dest)),
             (int)enc_data)) != 1) {
    ...
    DEBUG_SHMEM("[T_%d] Entering final with local_ctx %p tmp_buf %p + "
                ...
                tn, local_ctx, temp_cipher_text, AES_TAG_LEN, src,
                local_cipherlen);

    if ((res = EVP_EncryptFinal_ex(local_ctx,
                                   temp_cipher_text + AES_RAND_BYTES + src +
                                       local_cipherlen,
                                   &local_cipherlen)) != 1) {
    ...
    if ((res = EVP_CIPHER_CTX_ctrl(local_ctx, EVP_CTRL_GCM_GET_TAG, AES_TAG_LEN,
                                   temp_cipher_text + AES_RAND_BYTES +
                                       local_cipherlen + src)) != 1) {
    ...
    // free(temp_cipher_text);
  }
  for (count = 0; count < segment_count; count++) {
    position = count * (data + AES_TAG_LEN + AES_RAND_BYTES);
    memcpy(cipherbuf + position, enc_chunks[count],
           data + AES_RAND_BYTES + AES_TAG_LEN);
  }
  ...
  *cipherlen = temp_cipherlen;
  ...
  //   for (count  = 0; count < cipherlen; count++)
  //      fprintf(stderr, "%c", cipherbuf[count]);
  //   fprintf(stderr, "\n");
...
int shmemx_decrypt_single_buffer_omp(unsigned char *cipherbuf,
  ...
  // #pragma omp parallel for schedule (dynamic) default(none) private(max_data,
  // position, res, local_cipherlen, enc_data) shared(src, dest, openmp_enc_ctx,
  // stdout,stderr, segment_count, data, rbuf, cipherbuf, temp_cipherlen, proc,
  // bytes, gcm_key, enc_chunks, dec_chunks, plain_chunks)
  // num_threads(thread_no)
  for (count = 0; count < segment_count; count++) {
    memset(enc_chunks[count], 0, MAX_MSG_SIZE + OFFSET);
  ...
  for (count = 0; count < segment_count; count++) {
    if (count == segment_count - 1) {
      enc_data = (bytes - data * (count));
    }
    memcpy(enc_chunks[count],
           cipherbuf + (enc_data * count + AES_TAG_LEN + AES_RAND_BYTES),
           enc_data + AES_TAG_LEN + AES_RAND_BYTES);
  }
  enc_data = data;
...
#pragma omp parallel for schedule(dynamic) default(none)                       \
    ...
    shared(segment_count, stdout, stderr, openmp_dec_ctx, data, cipherbuf,     \
               rbuf, cipher_len, src, dest, bytes, proc, gcm_key, enc_chunks,  \
               dec_chunks, plain_chunks) num_threads(thread_no)
  for (count = 0; count < segment_count; count++) {
    ...
    position = count * (data + AES_TAG_LEN + AES_RAND_BYTES);
    int pos2 = count * data;
    unsigned char *temp_cipher_text = &(enc_chunks[count][0]);
    unsigned char *temp_decrypt_text =
        &(plain_chunks[count]); //+ (count * data);
    enc_data = data;
    max_data = data + AES_TAG_LEN;
    ...
    if ((res = EVP_DecryptInit_ex(local_ctx, NULL, NULL, NULL,
                                  temp_cipher_text)) != 1) {
    ...
    if ((res = EVP_DecryptUpdate(
             local_ctx, ((unsigned char *)(temp_decrypt_text + dest)),
             (int *)(&local_cipherlen), temp_cipher_text + AES_RAND_BYTES + src,
             (enc_data - AES_TAG_LEN))) != 1) {
    ...
    if ((res = EVP_DecryptFinal_ex(local_ctx,
                                   (&temp_decrypt_text[dest + local_cipherlen]),
    ...
    DEBUG_SHMEM("T_%d DecryptFinal_ex passed with text: %s\n", tn,
                temp_decrypt_text);
  }
  for (count = 0; count < segment_count; count++) {
    memcpy(rbuf + (count * data), plain_chunks[count], data);
  }
