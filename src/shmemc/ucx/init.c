 ...
 * worker tables
 */
inline static void allocate_xworkers_table(void) {
  proc.comms.xchg_wrkr_info = (worker_info_t *)calloc(
      proc.li.nranks, sizeof(*(proc.comms.xchg_wrkr_info)));
  shmemu_assert(proc.comms.xchg_wrkr_info != NULL,
                MODULE ": can't allocate memory for worker exchange");
}

inline static void deallocate_xworkers_table(void) {
  int pe;
...
inline static void get_globals_address_range(uint64_t *base_p,
                                             uint64_t *end_p) {
#ifdef __linux__
...
inline static void register_globals(void) {
  uint64_t g_base;
  uint64_t g_end;
  size_t len;
  ...
  shmemu_assert(s == UCS_OK, MODULE ": can't map global variables memory: %s",
                ucs_status_string(s));

  /* don't need allocator, variables already there */
}

inline static void deregister_globals(void) {
  ucs_status_t s;

  s = ucp_mem_unmap(proc.comms.ucx_ctxt, globals->mh);
  shmemu_assert(s == UCS_OK, MODULE ": can't unmap global memory: %s",
                ucs_status_string(s));
}
...
inline static void register_symmetric_heap(size_t heapno, mem_info_t *mip) {
  ucs_status_t s;
  ucp_mem_map_params_t mp;
  ucp_mem_attr_t attr;
  ...
  shmemu_assert(proc.heaps.heapsize[heapno] > 0,
                MODULE ": cannot register empty symmetric heap #%lu", hn);
  ...
  mp.flags = UCP_MEM_MAP_NONBLOCK | UCP_MEM_MAP_ALLOCATE;

  s = ucp_mem_map(proc.comms.ucx_ctxt, &mp, &mip->mh);
  shmemu_assert(s == UCS_OK,
                MODULE ": can't map memory for symmetric heap #%lu: %s", hn,
                ucs_status_string(s));
...
inline static void deregister_symmetric_heap(mem_info_t *mip) {
  ucs_status_t s;
...
inline static void opaque_rkeys_init(void) {
  size_t r;

  proc.comms.orks =
      (mem_opaque_t *)calloc(proc.comms.nregions, sizeof(mem_opaque_t));
  shmemu_assert(proc.comms.orks != NULL,
                MODULE ": can't allocate memory for opaque rkeys");

  for (r = 0; r < proc.comms.nregions; ++r) {
    proc.comms.orks[r].rkeys =
        (mem_opaque_rkey_t *)calloc(proc.li.nranks, sizeof(mem_opaque_rkey_t));
    shmemu_assert(proc.comms.orks[r].rkeys != NULL,
                  MODULE ": can't allocate memory for opaque rkeys");
  }
}

inline static void opaque_rkeys_finalize(void) {
  size_t r;
  int pe;
...
inline static void init_memory_regions(void) {
  size_t i;
...
inline static void register_memory_regions(void) {
  size_t hi;
...
inline static void deregister_memory_regions(void) {
  size_t hi;
...
#define ALLOC_INTERNAL_SYMM_VAR(_var)                                          \
  do {                                                                         \
    int i;                                                                     \
    const size_t nbytes = sizeof(*(_var)) * SHMEM_BARRIER_SYNC_SIZE;           \
                                                                               \
...
inline static void ucx_init_ready(void) {
  ucs_status_t s;
  ucp_params_t pm;

  s = ucp_config_read(NULL, NULL, &proc.comms.ucx_cfg);
  shmemu_assert(s == UCS_OK, MODULE ": can't read UCX config: %s",
                ucs_status_string(s));
  ...
  pm.field_mask =
      ...
      UCP_PARAM_FIELD_ESTIMATED_NUM_EPS | UCP_PARAM_FIELD_ESTIMATED_NUM_PPN;
  ...
  if (s != UCS_OK) {
    shmemu_fatal(MODULE ": can't initialize UCX: %s", ucs_status_string(s));
  }
...
inline static void ucx_ready(void) {
  /* don't need config info any more */
  ucp_config_release(proc.comms.ucx_cfg);
}

inline static void ucx_cleanup(void) { ucp_cleanup(proc.comms.ucx_ctxt); }
