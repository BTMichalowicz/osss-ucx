...
void shmemc_pmi_publish_worker(void) {
  ...
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx can't publish worker blob");
}

static const char *rkey_exch_fmt = "r:%lu:%d"; /* region, pe */

inline static void publish_one_rkeys(size_t r) {
  pmix_value_t v;
  void *packed_rkey;
  size_t rkey_len;
  pmix_status_t ps;
  const ucs_status_t s = shmemc_ucx_rkey_pack(
      proc.comms.regions[r].minfo[proc.li.rank].mh, &packed_rkey, &rkey_len);
...
#ifndef ENABLE_ALIGNED_ADDRESSES
inline static void publish_one_heap(size_t r) {
  pmix_status_t ps;
  pmix_value_t vb, vs;
...
void shmemc_pmi_exchange_workers(void) {
  ...
  for (pe = 0; pe < proc.li.nranks; ++pe) {
    ...
    shmemu_assert(ps == PMIX_SUCCESS,
                  MODULE ": PMIx can't find remote worker blob for PE %d", pe);
...
#ifndef ENABLE_ALIGNED_ADDRESSES
inline static void exchange_one_heap(size_t r, int pe) {
  pmix_status_t ps;
...
inline static void exchange_one_rkeys(size_t r, int pe) {
  pmix_status_t ps;
  pmix_value_t *vp = NULL;
  pmix_byte_object_t *bop;
...
inline static void exchange_all_rkeys(size_t r) {
  int pe;
...
inline static void exchange_one_rkeys_and_heaps(size_t r) {
  int pe;
...
void shmemc_pmi_barrier_all(bool collect_data) {
  ...
  ps = PMIx_Commit();
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx PMIx_Commit() failed: %s",
  ...
  shmemu_assert(ps == PMIX_SUCCESS,
                ...
                collect_data ? "true" : "false", PMIx_Error_string(ps));
}
...
inline static pmix_status_t pmix_init_wrapper(pmix_proc_t *pp) {
...
inline static pmix_status_t pmix_finalize_wrapper(void) {
#ifdef HAVE_PMIX_NO_INIT_HINTS
...
inline static void init_ranks(void) {
  pmix_status_t ps;
  pmix_value_t *vp = NULL; /* holds things we get from PMIx */

  /* we can get our own rank immediately */
  proc.li.rank = (int)my_pmix.rank;
  shmemu_assert(proc.li.rank >= 0, MODULE ": PMIx PE rank %d must be >= 0",
                proc.li.rank);

  ps = PMIx_Get(&wc_pmix, PMIX_JOB_SIZE, NULL, 0, &vp);
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx can't get program size: %s",
                PMIx_Error_string(ps));
  ...
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx can't get universe size: %s",
                PMIx_Error_string(ps));
...
inline static void init_peers(void) {
  pmix_status_t ps;
  pmix_value_t *vp = NULL; /* holds things we get from PMIx */
  ...
  if (ps == PMIX_SUCCESS) {
    if (vp->data.string != NULL) {
      size_t n;
      const int s = shmemu_parse_csv(vp->data.string, &proc.li.peers, &n);
...
static const pmix_status_t ERROR_TO_REPORT = PMIX_ERR_PROC_REQUESTED_ABORT;
...
static void notification_fn(size_t evhdlr_registration_id, pmix_status_t status,
  ...
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx can't abort: %s",
                PMIx_Error_string(ps));
 ...
 */
static void evhandler_reg_callbk(pmix_status_t status, size_t evhandler_ref,
                                 void *cbdata) {
...
static void init_event_handler(void) {
  ...
  active = -1;
  PMIx_Register_event_handler(&sp, 1, NULL, 0, notification_fn,
                              evhandler_reg_callbk, (void *)&active);

  while (active == -1) {
  }

  shmemu_assert(active == 0, MODULE ": PMIx global exit event handler "
                                    "registration failed");
}
...
void shmemc_pmi_client_abort(const char *msg, int status) {
  ...
  ps = PMIx_Notify_event(ERROR_TO_REPORT, &my_pmix, PMIX_RANGE_NAMESPACE, &si,
                         1, NULL, NULL);
...
void shmemc_pmi_client_finalize(void) {
  ...
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx can't finalize itself: %s",
                PMIx_Error_string(ps));
...
void shmemc_pmi_client_init(void) {
  ...
  shmemu_assert(ps == PMIX_SUCCESS, MODULE ": PMIx can't initialize itself: %s",
                PMIx_Error_string(ps));
